Hi 👋 My name is Chitresh Gyanani
==================================

[![](https://visitcount.itsvg.in/api?id=chitresh-code&icon=2&color=0)](https://visitcount.itsvg.in)

Software Engineer | GenAI | Data Science | Backend Systems
----------------------------------------------------------

🔭 Currently working on **enterprise and startup AI systems** (GenAI, RAG, realtime voice agents)  
👯 Open to collaborating on **AI/ML, GenAI, and backend-heavy products**  
🌱 Deeply exploring **AI Agents, RAG architectures, realtime speech systems, and ML pipelines**  
💬 Ask me about **Generative AI, LLMs, Data Science, and scalable backend systems**

---

### 👨🏻‍💻 About Me  

I’m a **Software Engineer and GenAI specialist** with hands-on experience building **production-grade AI platforms, SaaS backends, and ML systems**.  

My work spans:
- **LLM Engineering & RAG pipelines**
- **Realtime voice-to-voice & transcription systems**
- **Data science workflows (EDA, feature engineering, model evaluation)**
- **Enterprise analytics & AI governance platforms**
- **Secure developer tooling & backend infrastructure**

I’ve worked across **enterprise environments (UKG)** and **startup teams**, building systems that handle **real users, real data, cost tracking, security, and scale**.

```python
class Chitresh:
    def __init__(self):
        self.name = "Chitresh Gyanani"
        self.username = "Chitresh-code"
        self.location = "New Delhi, India"
        self.roles = [
            "Software Engineer",
            "GenAI Engineer",
            "Data Scientist",
            "Backend Developer"
        ]
        self.interests = [
            "LLM Engineering",
            "RAG Systems",
            "AI Agents",
            "Realtime AI",
            "ML Pipelines"
        ]
        self.web = "https://www.chitresh.in"

    def __str__(self):
        return self.name

me = Chitresh()
````

---

### 🚀 What I’m Working On

* **Enterprise AI Hub (UKG)**
  → RAG over SharePoint, MongoDB vector search, Polars analytics
  → **Realtime voice-to-voice & speech-to-text agents** (WebSocket, WebRTC)
  → Multi-agent orchestration with Semantic Kernel

* **ExtracTable (SaaS Backend)**
  → PDF table extraction with async processing
  → Token counting, cost tracking, billing-ready analytics
  → Django, Celery, PostgreSQL, Google Gemini

* **KeyNest (Open Source)**
  → Secure environment variable management for teams
  → AES-256 encryption, RBAC, audit logs
  → Django REST + Next.js

---

### 💻 Tech Stack

**Languages & Core**

![Python](https://img.shields.io/badge/python-3670A0?style=plastic\&logo=python\&logoColor=ffdd54)
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=plastic\&logo=c%2B%2B\&logoColor=white)
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=plastic\&logo=typescript\&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=plastic\&logo=javascript\&logoColor=%23F7DF1E)

**Backend & APIs**

![Django](https://img.shields.io/badge/django-%23092E20.svg?style=plastic\&logo=django\&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=plastic\&logo=fastapi)
![Flask](https://img.shields.io/badge/flask-%23000.svg?style=plastic\&logo=flask\&logoColor=white)
![NodeJS](https://img.shields.io/badge/node.js-6DA55F?style=plastic\&logo=node.js\&logoColor=white)

**AI / ML / GenAI**

![PyTorch](https://img.shields.io/badge/PyTorch-%23EE4C2C.svg?style=plastic\&logo=PyTorch\&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-%23FF6F00.svg?style=plastic\&logo=TensorFlow\&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-%23F7931E.svg?style=plastic\&logo=scikit-learn\&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-black?style=plastic)
![Neo4j](https://img.shields.io/badge/Neo4j-008CC1?style=plastic\&logo=neo4j\&logoColor=white)

**Databases & Infra**

![Postgres](https://img.shields.io/badge/postgres-%23316192.svg?style=plastic\&logo=postgresql\&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=plastic\&logo=mongodb\&logoColor=white)
![Redis](https://img.shields.io/badge/redis-%23DD0031.svg?style=plastic\&logo=redis\&logoColor=white)
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=plastic\&logo=docker\&logoColor=white)

**Frontend & UI**

![React](https://img.shields.io/badge/react-%2320232a.svg?style=plastic\&logo=react\&logoColor=%2361DAFB)
![Next JS](https://img.shields.io/badge/Next-black?style=plastic\&logo=next.js\&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/tailwindcss-%2338B2AC.svg?style=plastic\&logo=tailwind-css\&logoColor=white)

---

### 🌐 Connect With Me

<p>
<a href="https://www.linkedin.com/in/chitresh-gyanani"><img src="https://img.icons8.com/doodle/40/000000/linkedin--v2.png"/></a>
<a href="https://github.com/Chitresh-code"><img src="https://img.icons8.com/doodle/40/000000/github--v1.png"/></a>
<a href="https://stackoverflow.com/users/22677918/chitresh-gyanani"><img src="https://img.icons8.com/external-tal-revivo-color-tal-revivo/40/000000/external-stack-overflow-is-a-question-and-answer-site-for-professional-logo-color-tal-revivo.png"/></a>
<a href="https://twitter.com/chitreshgyanani"><img src="https://img.icons8.com/doodle/40/000000/twitter-squared--v2.png"/></a>
<a href="mailto:gychitresh1290@gmail.com"><img src="https://img.icons8.com/doodle/48/gmail-new.png"/></a>
<a href="https://www.chitresh.in/"><img src="https://img.icons8.com/plasticine/40/000000/resume.png"/></a>
</p>

---

### 📊 GitHub Stats

![](https://github-readme-stats.vercel.app/api?username=chitresh-code\&theme=dark\&hide_border=false\&include_all_commits=true)
![](https://github-readme-streak-stats.herokuapp.com/?user=chitresh-code\&theme=dark\&hide_border=false)
![](https://github-readme-stats.vercel.app/api/top-langs/?username=chitresh-code\&theme=dark\&hide_border=false\&layout=compact)

---

### ✍️ Random Dev Quote

![](https://quotes-github-readme.vercel.app/api?type=horizontal\&theme=tokyonight)
